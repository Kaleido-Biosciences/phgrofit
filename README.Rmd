...
annotated_data = phgrofit::meta_combine(phgrobiome_output = model_data,metadata = metadata,
...
7. simplify(): Summarizes the model fit by takeing the averages across distinct groups. 
...
```{r,eval= FALSE}
#This code condct spline interpolation from data called tidydata here and extracts the relevant parameters 
#to a data table called output. 10 randomly sampled graphs would be printed to the console for spot checking. 
...
**Input:**  
1. annotated_data: output of meta_combine, or simplify.
...
This function allows the user to simplify the data by grouping and summarising by distinct groups.   


**Input:**  

1. annotated_data: output of meta_combine.  
2. groups: This is a vector input containing the names of the columns that the data will be grouped by and the mean will subsequently be taken across these groups.  


**Output:**   

1. A data frame containing the averaged summarized fits across the groups provided by the user. The Sample.IDs have been changed to row names in order to ensure compatibility with the heatmapper function. 

#phgropro .txt input example
```{r}
biotek_export = "tests/testdata/phgropro_96_test.txt"
raw_data = read.delim(biotek_export,row.names = NULL,header = FALSE,col.names = 1:97,
